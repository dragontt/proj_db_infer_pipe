...
def main(argv):
    ...
    labels.append('baranski_singles_net_full')
    labels.append('cellCycle')
    labels.append('flyAtlas')
    labels.append('geneDoseChange2L')
    labels.append('genotypeByDiet')
    labels.append('gravityResponse')
    labels.append('lifeHistoryTraits')
    labels.append('postFastingOlfactory')
    labels.append('sexAntagonistic')
    labels.append('toxicogenomicsLead')
    fns = []
    for i in range(1,len(labels)):
        fns.append('fly_network_' + labels[i] + '_' + mapping_method + '/')
