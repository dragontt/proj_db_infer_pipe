...
def main(argv):
    ...
    fns.append('fly_network_singles_net_full_local_shrinkage/')
    fns.append('fly_network_singles_net_full_spearmanr/')
    fns.append('fly_network_singles_net_full_bart/')
    fns.append('fly_motif_inference/binding_network_motif_dbd_pid_only/')
    # fns.append('fly_motif_inference/binding_network_motif_dbd_pid_only2/')

    # figure setup
    colors = ['k--', 'k', 'r', 'g', 'b', 'm', 'c']
    labels = []
    labels.append('chance')
    labels.append('np_global')
    labels.append('np_local')
    labels.append('spearman_corr')
    labels.append('bart')
    labels.append('inferred_motif')
    # labels.append('inferred_motif2')
    ...
    for i in range(len(fns)):
        ...
        # eval_chip[i+1,:] = chip_support[2,:]
        # eval_pwm[i+1,:] = pwm_support[2,:]
