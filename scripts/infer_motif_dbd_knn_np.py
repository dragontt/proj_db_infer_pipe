...
"""
Infer a motif by first filtering database motifs based on the DNA binding domain percent identity (pid) within k nearest neighbors, and obtain the motif with highest rank order correlation of FIMO scan scores and the inferred network regulator-target scores.
"""
...
def parse_args(argv):
    ...
    parser.add_argument('-p', '--param_knn', dest='param_knn', type=int, default=10)
...
def main(argv):
            ...
            for i in range(len(lines)):
                ...
                if (pid >= PID_THRESHOLD):
                    if (not motif in motifs_not_use) and (motif in reg_corrs.keys()):
                        if len(candidate_motif) == 0:
                            candidate_motif.append(motif)
                            candidate_pid.append(pid)
                            candidate_corr.append(reg_corrs[motif])
                        elif len(candidate_motif) <= parsed.param_knn or pid == candidate_pid[len(candidate_motif)-1]:
                            candidate_motif.append(motif)
                            candidate_pid.append(pid)
                            candidate_corr.append(reg_corrs[motif])
                        else:
    ...
    # write data
    writer = open(parsed.dir_output + "motifs_dbd_knn_" + str(parsed.param_knn) + ".txt", "w")
