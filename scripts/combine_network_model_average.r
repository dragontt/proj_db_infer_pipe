...
# if (length(index) > 0){ net_np[index] <- net_np[index] - min(abs(net_np[index])) }
# index <- which(net_np < 0)
# if (length(index) > 0){ net_np[index] <- net_np[index] + min(abs(net_np[index])) }
net_np <- net_np / max(abs(net_np))

# index <- which(net_bart > 0)
# if (length(index) > 0){ net_bart[index] <- net_bart[index] - min(abs(net_bart[index])) }
# index <- which(net_bart < 0)
# if (length(index) > 0){ net_bart[index] <- net_bart[index] + min(abs(net_bart[index])) }
net_bart <- net_bart / max(abs(net_bart))

# net_comb <- compute.model.average.new(net_np, net_bart, c(1,1,1,1,1,1,0,0))
net_comb <- compute.model.average.new(net_np, net_bart, c(1,1,1,1,1,1,0.01,0.01))
