...
DIR_OUTPUT=$DIR_PROJ/output/yeast_network_dbd_pid
# TF_LIST=$DIR_RES/fly_aa_seq/${tf_list}
TF_LIST=$DIR_RES/yeast_aa_seq/${tf_list}
...
while read line; do
	...
	if [ -e $DIR_OUTPUT/$line ]; then
		...
		# # align against self TF DBD sequences of fly
		...
		# align against self TF DBD sequences of yeast
		bash $HOME/proj_db_infer_pipe/scripts/compt_dbd_pid.sh $line $DIR_PROJ $DIR_RES/yeast_aa_seq/yeast.dbd.fasta $DIR_RES/yeast_aa_seq/yeast.dbd.fasta $DIR_OUTPUT 0
		echo " ... Done"
	fi
done < $TF_LIST
