...
def parse_args(argv):
    ...
    parser.add_argument('-i', '-fn_inferred_pfms', dest='fn_inferred_pfms')
    parser.add_argument('-d', '-dir_inferred_pfms', dest='dir_inferred_pfms')
...
def main(argv):
    ...
    # parse FIRE output
    inferred_fire = {}
    lines = open(parsed.fn_inferred_pfms, 'r').readlines()
    for i in range(len(lines)):
        line = lines[i].strip().split('\t')
        inferred_fire[line[0]] = [line[2], line[3], line[4]]
    ...
    # run tomtom 
    results = numpy.array([['#TF','ortho_TF','Query_ID','Target_ID','Optimal_offset','p-value','E-value','q-value','Overlap','Query_consensus','Target_consensus','Orientation', 'mi', 'z-score', 'robustness']])
    ...
    for i in range(len(known_motifs)):
        ...
        if os.path.isfile(parsed.dir_inferred_pfms + tf):
            ...
            os.system('tomtom -no-ssc -oc '+ tmp_dir_tomtom +' -verbosity 1 -min-overlap 3 -dist ed -evalue -thresh '+ str(parsed.E_value_thresh) +' -bfile '+ bfile +' '+ parsed.dir_inferred_pfms + tf +' '+ parsed.fn_known_pfms)
            if os.path.isfile(tmp_dir_tomtom +'/tomtom.txt'):
                ...
                if (tomtom_result.size > 0) and (known_motifs[i,1] in tomtom_result[:,1]):
                    ...
                    if index.size > 0:
                        line = numpy.hstack((known_motifs[i,0], tf, tomtom_result[index[0],:], inferred_fire[tf]))
                        results = numpy.vstack((results, line))
