...
def main(argv):
    ...
    # run tomtom 
    results = numpy.array([['#Dmel_TF','ortho_TF','Query_ID','Target_ID','Optimal_offset','p-value','E-value','q-value','Overlap','Query_consensus','Target_consensus','Orientation']])
    ...
    for i in range(len(known_motifs)):
        ...
        if os.path.isfile(parsed.dir_inferred_pfms + tf):
            tmp_dir_tomtom = 'tmp_dir_tomtom_'+ parsed.species
